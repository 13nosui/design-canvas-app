...
void main() async {
  ...
  if (!await dir.exists()) {
    print('Directory lib/ui/page does not exist. Make sure you are running from the project root.');
    return;
  }

  print('Scanning dart files in ${dir.path} ...');
  final files = await dir.list(recursive: true).toList();
  final dartFiles = files.whereType<File>().where((f) => f.path.endsWith('.dart')).toList();

  // StatefulWidgetまたはStatelessWidgetを継承するクラスを見つける正規表現
  final classRegex = RegExp(r'class\s+([A-Z][a-zA-Z0-9_]*)\s+extends\s+(?:StatefulWidget|StatelessWidget)');
  ...
  for (final page in scannedPages) {
    ...
    final slug = page['path']!.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]+'), '_');
    ...
    outBuffer.writeln('    (context) => const CanvasSandbox(child: $className()),');
  ...
  print('Successfully generated $outPath with ${scannedPages.length} scanned routes!');
}
