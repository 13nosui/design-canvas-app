...
''';
  ...
  Future<void> _showCommitDialog(BuildContext context, ThemeControllerProvider themeController) async {
    final hexColor = '#${themeController.primaryColor.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}';
    final initialMessage = 'style: UIデザインの更新 (Color: $hexColor, Radius: ${themeController.borderRadius.toStringAsFixed(1)}px)';
          ...
          builder: (context, setState) {
            return AlertDialog(
              ...
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Text('現在のデザイン変更をローカルGitサーバー経由でコミット＆プッシュします。\\nコミットメッセージを編集してください：'),
                  const SizedBox(height: 16),
                  TextField(
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        if (response.statusCode == 200) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('✅ Successfully Pushed to GitHub!')),
                          );
                        } else {
                        ...
                        Navigator.of(diagContext).pop();
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('❌ サーバーに接続できません (local_git_server.dart を起動していますか？)\\n$e'), backgroundColor: Colors.red),
                        );
                      }
                    },
