        ...
        sections: z
          .array(
            z.object({
              label: z
                .string()
                .describe('セクションラベル (例: "アクション", "表示情報", "状態")'),
              body: z
                .string()
                .describe('セクションの中身を 1〜2 文で具体的に'),
            })
          )
          .min(2)
          .max(4)
          .describe('この画面固有のセクション 2〜4 件 (画面ごとに必ず異なる内容)'),
      })
    )
    .min(3)
    .max(5)
...
- screens: 実際にチームが画面リストを書くときに使えるレベル。各 screen には
  その画面固有の sections[] を 2〜4 件必ず含める。label は「アクション」
  「表示情報」「状態」「主要コンポーネント」などから画面ごとに選び、
  body はその画面でしか通用しない具体的な内容にする。絶対に画面間で
  sections を使い回さないこと
